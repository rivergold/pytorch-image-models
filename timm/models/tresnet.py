...
default_cfgs = {
    ...
    _cfg(
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-tresnet/tresnet_m_80_8-dbc13962.pth'
    ),
    'tresnet_l':
    _cfg(
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-tresnet/tresnet_l_81_5-235b486c.pth'
    ),
    'tresnet_xl':
    _cfg(
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-tresnet/tresnet_xl_82_0-a2d51b00.pth'
    ),
    'tresnet_m_448':
    _cfg(
        ...
        pool_size=(14, 14),
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-tresnet/tresnet_m_448-bc359d10.pth'
    ),
    'tresnet_l_448':
    _cfg(
        input_size=(3, 448, 448),
        pool_size=(14, 14),
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-tresnet/tresnet_l_448-940d0cd1.pth'
    ),
    'tresnet_xl_448':
    _cfg(
        input_size=(3, 448, 448),
        pool_size=(14, 14),
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-tresnet/tresnet_xl_448-8c1815de.pth'
    )
}
...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if stride == 1:
            self.conv1 = conv2d_iabn(inplanes,
                                     planes,
                                     stride=1,
                                     act_param=1e-3)
        else:
        ...
        self.se = SEModule(
            ...
            reduction_channels=reduction_chs) if use_se else None
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.se = SEModule(
            planes, reduction_channels=reduction_chs) if use_se else None

        self.conv3 = conv2d_iabn(planes,
                                 planes * self.expansion,
                                 kernel_size=1,
                                 stride=1,
                                 act_layer="identity")
...
class TResNet(nn.Module):
    def __init__(self,
        ...
        layer1 = self._make_layer(BasicBlock,
                                  ...
                                  stride=1,
                                  use_se=True,
                                  aa_layer=aa_layer)  # 56x56
        layer2 = self._make_layer(BasicBlock,
                                  self.planes * 2,
                                  layers[1],
                                  stride=2,
                                  use_se=True,
                                  aa_layer=aa_layer)  # 28x28
        layer3 = self._make_layer(Bottleneck,
                                  self.planes * 4,
                                  layers[2],
                                  stride=2,
                                  use_se=True,
                                  aa_layer=aa_layer)  # 14x14
        layer4 = self._make_layer(Bottleneck,
                                  self.planes * 8,
                                  layers[3],
                                  stride=2,
                                  use_se=False,
                                  aa_layer=aa_layer)  # 7x7

        # body
        self.body = nn.Sequential(
            OrderedDict([('SpaceToDepth', space_to_depth), ('conv1', conv1),
        ...
        self.feature_info = [
            ...
            dict(num_chs=self.planes * 4 * Bottleneck.expansion,
                 reduction=16,
                 module='body.layer3'),
            dict(num_chs=self.planes * 8 * Bottleneck.expansion,
                 reduction=32,
                 module='body.layer4'),
        ]
...
def _create_tresnet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(TResNet,
                                variant,
                                default_cfg=default_cfgs[variant],
                                pretrained=pretrained,
                                feature_cfg=dict(out_indices=(1, 2, 3, 4),
...
def tresnet_m_448(pretrained=False, **kwargs):
    model_kwargs = dict(layers=[3, 4, 11, 3], **kwargs)
    return _create_tresnet('tresnet_m_448',
                           pretrained=pretrained,
                           **model_kwargs)
...
def tresnet_l_448(pretrained=False, **kwargs):
    model_kwargs = dict(layers=[4, 5, 18, 3], width_factor=1.2, **kwargs)
    return _create_tresnet('tresnet_l_448',
                           pretrained=pretrained,
                           **model_kwargs)
...
def tresnet_xl_448(pretrained=False, **kwargs):
    model_kwargs = dict(layers=[4, 5, 24, 3], width_factor=1.3, **kwargs)
    return _create_tresnet('tresnet_xl_448',
                           pretrained=pretrained,
                           **model_kwargs)
