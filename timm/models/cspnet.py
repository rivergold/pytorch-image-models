...
default_cfgs = {
    ...
    _cfg(
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/cspresnet50_ra-d3e8d487.pth'
    ),
    'cspresnet50d':
    _cfg(url=''),
    'cspresnet50w':
    ...
    _cfg(
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/cspresnext50_ra_224-648b4713.pth',
    ...
    _cfg(
        url=
        'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/cspdarknet53_ra_256-d05c7c21.pth'
    ),
    'cspdarknet53_iabn':
    _cfg(url=''),
    'darknet53':
...
model_cfgs = dict(cspresnet50=dict(stem=dict(out_chs=64,
                                             kernel_size=7,
                                             stride=2,
                                             pool='max'),
                                   stage=dict(
                  ...
                  cspresnet50d=dict(stem=dict(out_chs=[32, 32, 64],
                                              kernel_size=3,
                                              stride=2,
                                              pool='max'),
                                    stage=dict(
                  ...
                  cspresnet50w=dict(stem=dict(out_chs=[32, 32, 64],
                                              kernel_size=3,
                                              stride=2,
                                              pool='max'),
                                    stage=dict(
                  ...
                  cspresnext50=dict(stem=dict(out_chs=64,
                                              kernel_size=7,
                                              stride=2,
                                              pool='max'),
                                    stage=dict(
                  ...
                  cspdarknet53=dict(stem=dict(out_chs=32,
                                              kernel_size=3,
                                              stride=1,
                                              pool=''),
                                    stage=dict(
                  ...
                  darknet53=dict(stem=dict(out_chs=32,
                                           kernel_size=3,
                                           stride=1,
                                           pool=''),
                                 stage=dict(
...
class CrossStage(nn.Module):
    ...
    def forward(self, x):
        ...
        xb = self.blocks(xb)
        out = self.conv_transition(
            torch.cat([xs, self.conv_transition_b(xb)], dim=1))
        return out
...
def _cfg_to_stage_args(cfg, curr_stride=2, output_stride=32,
    ...
    if 'cross_linear' in cfg and not isinstance(cfg['cross_linear'],
                                                (list, tuple)):
...
class CspNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.head = ClassifierHead(in_chs=prev_chs,
...
def _create_cspnet(variant, pretrained=False, **kwargs):
    ...
    return build_model_with_cfg(CspNet,
                                variant,
                                pretrained,
...
def cspresnext50_iabn(pretrained=False, **kwargs):
    ...
    return _create_cspnet('cspresnext50_iabn',
...
def cspdarknet53_iabn(pretrained=False, **kwargs):
    norm_layer = get_norm_act_layer('iabn')
    return _create_cspnet('cspdarknet53_iabn',
                          pretrained=pretrained,
                          block_fn=DarkBlock,
                          norm_layer=norm_layer,
                          **kwargs)
